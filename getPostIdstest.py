...
class TestGetPostIds(unittest.TestCase):
    ...
    def test_getPostIds_normalBehavior(self):
        ...
        output = getPostIds(self.db,self.offset,self.limit)
    ...
    def test_getPostIds_Exception(self):
        self.assertRaises(AttributeError,getPostIds,None,self.offset,self.limit)

    def test_getPostIds_wronglimit(self):
        self.insertPosts("publish", "normal") #fixture : inserting something in db for testing
        output = getPostIds(self.db,self.offset,0)
        self.assertNotEqual(self.arr,output)
        
    def test_getPostIds_wrongoffset(self):
        self.insertPosts("publish", "normal") #fixture : inserting something in db for testing
        output = getPostIds(self.db,200,self.limit)
        self.assertNotEqual(self.arr,output)
